 ...
 */
class Evaluator {
  constructor (grammar, transforms, context, relativeContext, parentMap, promise = Promise) {
  ...
  evalLambda (ast) {
    const params = ast.params.map(param=>param.value)
    ...
    return function() {
      ...
      params.forEach((name, idx)=>{
        if(idx < arguments.length) {
          lambdaContext[name] = arguments[idx];
        }
      })
      const evalInst = new Evaluator(grammar, transforms, lambdaContext, null, parentMap, PromiseSync)
      const result =  evalInst.eval(ast.right)
  ...
  _filterRelative(subject, expr) {
    ...
    subject.forEach((elem) => {
      const evalInst = new Evaluator(this._grammar, this._transforms, this._context, elem, this.parentMap)
