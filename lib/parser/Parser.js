...
class Parser {
  ...
  addToken(token) {
    ...
    if (state.subHandler) {
      ...
      throw new Error(`Token ${token.raw} (${token.type}) unexpected during state ${this._state} in expression: ${this._exprStr}`)
    }
    return false
  }
  ...
  /**
   * Indicates whether the expression tree contains a parent path identifier.
   * @returns {boolean} true if a parent identifier exists false otherwise.
   */
  isFromParent () {
