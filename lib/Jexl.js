...
class Jexl {
   ...
   */
  eval (expression, context = {}, parentMap) {
    return this._eval(expression, context, parentMap, Promise)
  }
   ...
   * @throws {*} on error
   */
  evalSync (expression, context = {}, parentMap) {
    const res = this._eval(expression, context, parentMap, PromiseSync)
   ...
   */
  _eval (expression, context, parentMap, promise) {
    const grammar = this._getGrammar()
    const parser = new Parser(grammar)
    const evaluator = new Evaluator(grammar, this._transforms, context, undefined, parentMap, promise)
